...
dir$root <- getwd()
#dir$source   <- paste(dir$root,"/AT_raw.csv",sep="")
dir$output   <- paste(dir$root,"/data_clean",sep="")
...
df <-rename(df, treatment = sys_block_set_1)

# select related columns 

names <- colnames(df)
apple_col <- names[grepl("^apple", names)]

df1 <- df [, c('Version', 'id', 'treatment', apple_col)]


# reshape data to wide format one id with 6 task

df2 <- pivot_longer(df1,
                   cols = starts_with("apple_Random"),
                   names_to = "task",
                   names_prefix = "apple_Random",
                   values_to = "choice",
                   values_drop_na = TRUE
)
# cast task from character to numeric
df2$task <- as.numeric(df2$task)


############ Change exp_design to standard wide ############
...
df2$id[df2$id==6] <- 16 # to make r identify id = 6 not as 60+... 

df_merge = merge(df2, exp_design, by.x=c('id','task', 'choice'), by.y=c('Version','Task','Concept'),sort = T)
...
df_merge4<- df_merge3[ ,-c(6:8)]

# create dummy 

df_merge5 <- dummy_cols(df_merge4,
                        select_columns = c( 'treatment','Att.1...Production.Method_1','Att.1...Production.Method_2'
...
drop <- c( 'Att.1...Production.Method_1', 'Att.1...Production.Method_2',
           ...
           'Att.2...Price_1','Att.2...Price_2',
           'Att.2...Price_3', 'Att.2...Price_4',
           'instore_1','instore_2','instore_3','instore_4','task'
           ) 
...
######select participants
df_apple<- df_clean3[df_clean3$id >= 1, ]


# export clean data in data_clean
OutPath<- "~/Desktop/crispr_r/data_clean/df_apple.csv"
